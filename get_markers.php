...
try {
    ...
    $queryLog = "
        INSERT INTO marker_request_log (
            lat_north, lat_south, lng_east, lng_west, party, township,
    ...
    //         p.full_township AS township, p.precinct AS precinct, p.city_council_ward AS ward, 
    //         p.county_supervisor AS supervisor, p.voterstatus AS voterstatus, p.regn_num AS voterid,
    //         p.strong_voter AS strong_voter, p.young_strong_voter AS young_strong_voter,
    //         p.needs_ride_to_poll AS needs_ride_to_poll, p.township_trustee_clerk AS township_trustee_or_clerk,
    ...
    //      AND p.party = ANY(\$5)
    //      AND (%s = ANY(string_to_array(\$6, ',')) OR \$6 = 'all' OR \$6 = '')"
    ...
    $query2 = sprintf(
        "SELECT 
            ...
            LEFT JOIN persons4 p
                ...
                FIXED UNIT MATCHING LOGIC
                ------------------------------- */
                AND (NULLIF(a.unit_type, '') IS NOT DISTINCT FROM NULLIF(p.unit_type, ''))
                AND (NULLIF(a.unit_num, '') IS NOT DISTINCT FROM NULLIF(p.unit_num, ''))
